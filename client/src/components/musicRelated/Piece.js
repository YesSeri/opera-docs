...
function Piece(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get(`/api/pieces/${id}`)
      .then(response =>{
        setDownloadlink(`https://singcademy.com/wp-content/uploads/pdfsToBeAccessed/${response.data.file_title}`)
        return response
      })
      .then((response) => {
            ...
            <Container fluid>
              ...
              <Row>
                <Col style={{ width: '32rem' }}>
                  ...
                  <p>{description}</p>
                  <p>Either view the score here, <a href={downloadLink}>download it</a></p>
                </Col>
              </Row>
            </Container>
          </>
  ...
  return (
    <>
      {data ? PieceInfo() : null}
      <PdfFrame downloadLink={downloadLink} /> 
    </>
  );
}
